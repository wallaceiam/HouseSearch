...
export const getStaticProps: GetStaticProps = async (context) => {
  const { localAuthoritySlug } = context.params ?? {};

  const la = await getLocalAuthorityById((localAuthoritySlug as string) ?? "");
  const schoolTypes = await getSchoolTypes();
  const schools = await getSchoolsForLocalType(localAuthoritySlug as string);
  return {
    props: {
      localAuthority: la,
      schoolTypes,
    ...
    <Layout>
      ...
      <main className="pt-8 pb-16">
        <div className="mx-auto max-w-7xl sm:px-6 lg:px-8">
          ...
          <SchoolTable schools={displayedSchools} />
        </div>
      </main>
    </Layout>
  );
