...
const Popup = ({ school }: ISchoolPopup) => {
  ...
  const link = useMemo(() => {
    ...
    return `/schools/${s(localAuthority)}/${s(minorGroup)}/${id}/${s(name)}`;
  }, [id, name, localAuthority, minorGroup]);

  const type = useMemo(() => {
    return [
      (isTypeFlag & 8) !== 0 ? "Post 16" : undefined,
        ...
        <div className="flex">
          ...
          <div>
            ...
            <ul className="text-sm font-light">
              ...
              <li className="flex items-start mb-2">
                <MapPinIcon className="mr-1 min-w-4 min-h-4 w-4 h-4 font-semibold text-gray-400" />
                <span>{address}</span>
              </li>
              {telephone && (
                <li className="flex items-start mb-2">
